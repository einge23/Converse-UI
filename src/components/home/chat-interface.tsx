...
export function ChatInterface({
    ...
    if (prevMessages && !isLoadingMore) {
      if (page === 1) {
        if (
          ...
          JSON.stringify(allMessages) !== JSON.stringify(prevMessages.messages)
        ) {
  ...
  const formatTimestamp = useCallback((timestamp: string) => {
    ...
    // helper to format just the time in h:mm AM/PM
    ...
    if (dt >= startOfToday) {
      ...
      return `yesterday at ${fmtTime(dt)}`;
    } else {
  ...
  const combinedMessages = useMemo(() => {
    ...
    const uniqueMessages = allMsgs.filter(
      (message, index, arr) =>
        arr.findIndex((m) => m.message_id === message.message_id) === index
    );
  ...
  const messageElements = useMemo(() => {
    ...
    return reversedMessages.map((message, index) => {
      ...
      const isFirstInGroup =
        originalIndex === 0 ||
        combinedMessages[originalIndex - 1].sender_id !== message.sender_id ||
        new Date(message.created_at).getTime() -
          new Date(combinedMessages[originalIndex - 1].created_at).getTime() >
          300000;
          ...
          <div className="relative">
            ...
            <div
              className={cn(
                ...
                friend.status === "do_not_disturb" && "bg-red-500",
                friend.status === "offline" && "bg-muted"
              )}
            />
          </div>
          <div>
            <h2 className="font-semibold text-base">{friend.username}</h2>
      ...
      <div className="p-4">
        <div className="flex items-center gap-2 rounded-lg bg-muted p-2">
          ...
          <div className="flex shrink-0 items-center gap-1">
            ...
            <Button
              size="icon"
              className={cn("h-8 w-8", !newMessage.trim() && "opacity-50")}
  ...
  ({
    ...
    return (
      ...
      >
        ...
        <div className={cn("flex flex-col flex-1", !isFirstInGroup && "ml-14")}>
          {isFirstInGroup && (
